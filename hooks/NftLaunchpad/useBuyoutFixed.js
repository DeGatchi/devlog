...
import { useBuyoutFixedContract } from "../useContract";

const useBuyoutFixed = () => {
  const { signer, network } = Connection.useContainer();

  const getBuyoutEvents = async (_contract) => {
    const SALE = await useBuyoutFixedContract(_contract);
    try {
      // get the event()
  ...
  const init = async (
    ...
    try {
      console.log(
        ...
        Number(ethers.utils.parseUnits(_price)),
        Number(_totalSupply),
        _merkleRoot,
        []
      );
      const tx = await SALE?.connect(signer).init(
        ...
        Number(_startDelay * 86400),
        Number(_duration * 86400),
        ethers.utils.parseUnits(_price),
        Number(_totalSupply),
        _merkleRoot,
        []
      );
  ...
  const buyout = async (_contract, _merkleProof, _amount) => {
    ...
    try {
      ...
      const [buyer, amount, bonus, cost] = event.args;
      return [buyer, amount, bonus, cost];
    } catch (err) {
      console.log(err);
      return err;
    }
  ...
  // returns: [initialised, _info, _sale, _bulkBonuses]
  const getBuyoutFixedSaleDetails = async (_contract) => {
    const SALE = await useBuyoutFixedContract(_contract);
    try {
      const tx = await SALE?.connect(signer).getSaleDetails();
      return tx;
  ...
  const getBuyoutFixedOwner = async (_contract) => {
    const SALE = await useBuyoutFixedContract(_contract);
    try {
  ...
  return {
    ...
    getBulkBonus,
    getSaleDetails,
    getBuyoutFixedSaleDetails,
    getBuyoutFixedOwner
  };
};

export default useBuyoutFixed;
